...
// Connexion au serveur MySQL (sans DB pour pouvoir la créer)
...
if (!$hasTable('users')) {
    ...
    // S'assurer des colonnes importantes (idempotent)
    ...
    foreach (['date' => 'DATE', 'start_time' => 'TIME', 'end_time' => 'TIME'] as $col => $type) {
        if ($hasColumn('slots', $col)) {
            try {
                ...
                // Si des données bloquent, on laisse tel quel.
            }
        } else {
...
// Index d’unicité business : (coach_id, date, start_time)
...
if (!$hasTable('reservations')) {
    ...
    // s'assurer des colonnes essentielles
...
ensureCoach($pdo, 'Nadia',   'Coach',  'nadia.coach@example.com',   '0600000001', 'Marne-la-Vallée');
ensureCoach($pdo, 'Sabrina', 'Coach',  'sabrina.coach@example.com', '0600000002', 'Paris / 92-95');
