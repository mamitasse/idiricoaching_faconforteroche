 ...
 */
?>
<section class="nadia-page">

    <!-- En-tête -->
    <header class="nadia-header">
        <h1>Nadia</h1>
        <p>Coach (Marne la Vallée) — coaching personnalisé et cours de fitness.</p>
    </header>

   
  

    <section class="nadia-carousel">
        <h2>Galerie</h2>

        <div class="carousel" id="nadiaCarousel" tabindex="0" aria-label="Galerie de photos de Nadia">
            <div class="carousel-viewport">
                <div class="carousel-track">
                    <?php foreach (($gallery ?? []) as $item):
                        $src     = BASE_URL . ltrim((string)($item['src'] ?? ''), '/');
                        $alt     = htmlspecialchars($item['alt'] ?? 'demo Nadia', ENT_QUOTES, 'UTF-8');
                        $caption = htmlspecialchars($item['caption'] ?? '', ENT_QUOTES, 'UTF-8');
                    ?>
                        <figure class="carousel-slide">
                            <img loading="lazy" src="<?= $src ?>" alt="<?= $alt ?>">
                            <?php if ($caption !== ''): ?>
                                <figcaption class="nadia-legend"><?= $caption ?></figcaption>
                            <?php endif; ?>
                        </figure>
                    <?php endforeach; ?>
                </div>
    ...
    <script>
        (function() {
            const root = document.getElementById('nadiaCarousel');
            if (!root) return;
